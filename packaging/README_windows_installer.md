...
推送与 `src/version.h` 一致的 `v*` 标签时，例如 `v2026.06.12`，CI 会自动创建或更新同名 GitHub Release，并上传；普通分支 push 和 PR 只生成 Actions artifact，不发布 Release。
...
`lis.ps1 build` 会从 `src\version.h` 自动读取版本号并生成 `out\windows\installer\LISWorkbench-Setup.exe`，但不会生成更新 zip/manifest，也不会在默认 `auto` 策略下下载远程 LabelPrint 包。`lis.ps1 package` 和 `lis.ps1 rebuild-package` 会从 `src\version.h` 自动读取版本号；需要覆盖时可传 `-AppVersion vYYYY.MM.DD`。
...
& "C:\Program Files (x86)\NSIS\makensis.exe" /DAPP_VERSION=v2026.06.12 /DAPP_EXE=lis_workbench.exe /DBUILD_DIR=..\build\main-app\Release /DOUTPUT_DIR=..\out\windows\installer /DOUTPUT_NAME=LISWorkbench-Setup.exe packaging\LISWorkbench.nsi
```
...
- `auto`：默认策略，`package/rebuild-package` 使用 GitHub；`build/run` 不强制 LabelPrint 来源，交给 CMake 默认查找逻辑处理。
