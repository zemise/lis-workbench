...
MenuDrawText *addMenuDrawText(const wchar_t *text, bool topLevel) {
  g_menuDrawTexts.push_back(std::make_unique<MenuDrawText>(
      MenuDrawText{text ? text : L"", topLevel}));
...
void applyFontToChildren(HWND root, HFONT font) {
  ...
  EnumChildWindows(
      ...
      [](HWND child, LPARAM p) -> BOOL {
        if (GetDlgCtrlID(child) == ID_STATUS)
          return TRUE;
...
void rebuildUiFont(int fontSize) {
  ...
  if (tb) {
    ...
    SendMessageW(g_ctx.mainWindow, WM_SIZE, 0,
                 MAKELPARAM(rc.right - rc.left, rc.bottom - rc.top));
  }
  if (g_ctx.mainWindow)
    DrawMenuBar(g_ctx.mainWindow);
  broadcastFontChanged();
  if (oldFont)
    DeleteObject(oldFont);
  if (oldMenuFont)
    DeleteObject(oldMenuFont);
}
...
LRESULT CALLBACK mdiChildProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
  ...
  case WM_CREATE: {
    ...
    CreateWindowExW(0, L"STATIC", title, WS_CHILD | WS_VISIBLE | SS_CENTER, 0,
                    0, 0, 0, hwnd, nullptr, g_ctx.instance, nullptr);
    return 0;
  }
  case WM_SIZE: {
...
HWND createMdiChild(const wchar_t *title) {
  ...
  HWND child = reinterpret_cast<HWND>(SendMessageW(
      g_ctx.mdiClient, WM_MDICREATE, 0, reinterpret_cast<LPARAM>(&mcs)));
  SendMessageW(g_ctx.mdiClient, WM_MDIMAXIMIZE, reinterpret_cast<WPARAM>(child),
               0);
  return child;
}
...
HWND getActiveMdiChild() {
  ...
  return reinterpret_cast<HWND>(
      SendMessageW(g_ctx.mdiClient, WM_MDIGETACTIVE, 0, 0));
}
...
int toolbarCommandForMdiChild(HWND child) {
  if (!child)
    return 0;
  wchar_t title[256]{};
  if (!GetWindowTextW(child, title, 256))
    return 0;
  if (lstrcmpW(title, L"标本签收中心") == 0)
    return IDM_TOOL3;
  if (lstrcmpW(title, L"常规报告") == 0)
    return IDM_TOOL2;
  ...
  if (lstrcmpW(title, L"质控分析") == 0)
    return IDM_QCONTROL;
  if (lstrcmpW(title, L"门诊查询") == 0)
    return IDM_TOOL5;
  if (lstrcmpW(title, L"常用电话") == 0)
    return IDM_TOOL6;
  return 0;
}
...
void closeActiveMdiChild() {
  HWND active = getActiveMdiChild();
  if (active)
    SendMessageW(g_ctx.mdiClient, WM_MDIDESTROY,
                 reinterpret_cast<WPARAM>(active), 0);
  updateToolbarState();
}

// ── placeholder factories (to be replaced with real modules) ────

HWND create_tool4_placeholder(const ModuleContext &) {
  return createMdiChild(L"工具4");
}

// ── module registry ─────────────────────────────────────────────

const ModuleDef g_modules[] = {
    {L"Query", L"检验管理", L"检验结果查询(&Q)...", IDM_QUERY,
     create_query_module},
    {L"Blood", L"检验管理", L"输血结果查询(&B)...", IDM_BLOOD,
     create_blood_module},
    {L"QualityControl", L"检验管理", L"质控分析(&C)", IDM_QCONTROL,
     create_quality_control_module},
    {L"Barcode", L"工具", L"已签收条码查询(&1)...", IDM_TOOL1,
     create_barcode_module},
    {L"MchcCorrection", L"工具", L"脂血MCHC校正(&2)", IDM_TOOL4,
     create_mchc_correction_module},
    {L"OutpatientQuery", L"工具", L"门诊查询(&3)", IDM_TOOL5,
     create_outpatient_query_module},
    {L"PhoneDirectory", L"工具", L"常用电话(&4)", IDM_TOOL6,
     create_phone_directory_module},
    {L"HivStatistics", L"统计分析管理", L"HIV 抗体检测统计(&1)", IDM_STAT1,
     create_hiv_statistics_module},
    {L"EmergencyStatistics", L"统计分析管理", L"急诊样本统计(&2)", IDM_STAT2,
     create_emergency_statistics_module},
    {L"ImmuneDuplicateStatistics", L"统计分析管理", L"免疫重复项目统计(&3)",
     IDM_STAT3, create_immune_duplicate_statistics_module},
    {L"BackupBloodStatistics", L"统计分析管理", L"备血统计(&4)", IDM_STAT4,
     create_backup_blood_statistics_module},
    {L"MassiveTransfusionStatistics", L"统计分析管理", L"大量输血统计(&5)",
     IDM_STAT5, create_massive_transfusion_statistics_module},
    {L"TransfusionOrderStatistics", L"统计分析管理", L"输血单统计(&6)",
     IDM_STAT6, create_transfusion_order_statistics_module},
    {L"TatStatistics", L"统计分析管理", L"检验周转时间统计(&7)", IDM_STAT7,
     create_tat_statistics_module},
    {L"ScheduledResultCheck", L"定时任务", L"定时核查结果(&1)",
     IDM_SCHEDULED_RESULT_CHECK, create_scheduled_result_check_module},
    {L"Settings", L"系统", L"系统设置(&S)...", IDM_SETTINGS,
...
void updateStatusBarParts(HWND sb, int clientWidth) {
  ...
  int parts[] = {clientWidth * 30 / 100, clientWidth - ipW - timeW,
                 clientWidth - timeW, clientWidth};
...
UpdateSourceConfig loadUpdateSourceConfig() {
  ...
  cfg.manifestUrl =
      search::load_module_str(lis_update::kConfigSection, L"ManifestUrl",
                              lis_update::kDefaultGithubManifestUrl);
  if (cfg.manifestUrl.empty()) {
    cfg.manifestUrl = lis_update::kDefaultGithubManifestUrl;
  }
  cfg.folderPath =
      search::load_module_str(lis_update::kConfigSection, L"FolderPath", L"");
  return cfg;
}
...
std::wstring programDataUpdateCacheDir() {
  ...
  if (needed > 0) {
    ...
    DWORD written =
        GetEnvironmentVariableW(L"ProgramData", buffer.data(), needed);
...
bool launchUpdaterForPackage(HWND hwnd, const std::wstring &package_path) {
  ...
  const std::wstring updater_path =
      lis_update::join_update_path(app_dir, UPDATE_UPDATER_EXE_NAME);
  if (!localFileExists(updater_path)) {
    MessageBoxW(hwnd, L"安装目录中未找到 Updater.exe。", L"安装更新",
                MB_ICONERROR);
    return false;
  }
  ...
  HINSTANCE started =
      ShellExecuteW(nullptr, L"open", updater_path.c_str(), args.c_str(),
...
void startManualUpdateCheck(HWND hwnd) {
  if (g_manualUpdateChecking) {
    MessageBoxW(hwnd, L"正在检查更新，请稍候。", L"检查更新",
                MB_ICONINFORMATION);
    return;
  }
  ...
  if (cfg.sourceType == lis_update::kSourceHttp) {
    if (cfg.manifestUrl.empty()) {
      MessageBoxW(hwnd, L"请先在系统设置中填写 HTTP manifest URL。",
                  L"检查更新", MB_ICONWARNING);
      return;
    }
  } else if (cfg.folderPath.empty()) {
    MessageBoxW(hwnd, L"请先在系统设置中填写共享文件夹路径。", L"检查更新",
                MB_ICONWARNING);
    return;
  }
  ...
  const bool queued = g_manualUpdateTask.start<ManualUpdateCheckDone>(
      [cfg, cache_dir] {
        ...
        if (cfg.sourceType == lis_update::kSourceHttp) {
          source =
              std::make_unique<lis_update::HttpUpdateSource>(cfg.manifestUrl);
        } else {
          source =
              std::make_unique<lis_update::FolderUpdateSource>(cfg.folderPath);
        }
      ...
      [hwnd](std::optional<ManualUpdateCheckDone> done,
        ...
        if (error || !done) {
          ...
          MessageBoxW(hwnd, L"检查更新时发生后台任务异常。", L"检查更新失败",
                      MB_ICONERROR);
          return;
        }
  ...
  if (!queued) {
    g_manualUpdateChecking = false;
    setMainStatusText(L"就绪");
    MessageBoxW(hwnd, L"无法启动更新检查后台任务。", L"检查更新失败",
                MB_ICONERROR);
  }
}
...
void showManualUpdateCheckResult(HWND hwnd, const ManualUpdateCheckDone &done) {
  ...
  if (!done.ok) {
    MessageBoxW(hwnd, search::utf8_to_wide(done.error).c_str(), L"检查更新失败",
                MB_ICONERROR);
    return;
  }
...
bool shouldAutoCheckUpdateToday() {
  ...
  const std::wstring last = search::load_module_str(lis_update::kConfigSection,
                                                    L"LastCheckTime", L"");
  return last != today;
}

void markAutoCheckUpdateToday() {
  search::save_module_str(lis_update::kConfigSection, L"LastCheckTime",
                          todayUpdateStamp());
}
...
void startAutoUpdateCheck(HWND hwnd) {
  ...
  if (cfg.sourceType == lis_update::kSourceHttp && cfg.manifestUrl.empty())
    return;
  if (cfg.sourceType != lis_update::kSourceHttp && cfg.folderPath.empty())
    return;
  ...
  g_autoUpdateTask.start<AutoUpdateCheckDone>(
      [cfg] {
        ...
        if (done.ok && lis_update::compare_version_strings(
                           manifest.version, search::kVersion) > 0) {
          done.updateAvailable = true;
          done.version = manifest.version;
        }
        return done;
      },
      [hwnd](std::optional<AutoUpdateCheckDone> done,
             std::exception_ptr error) {
        if (!IsWindow(hwnd) || error || !done)
          return;
...
void showAutoUpdateCheckResult(HWND hwnd, const AutoUpdateCheckDone &done) {
  if (!done.ok || !done.updateAvailable)
    return;
  const std::wstring message = L"发现新版本：" +
                               search::utf8_to_wide(done.version) +
                               L"\r\n是否现在下载并安装更新？";
...
void appendOwnerDrawItem(HMENU menu, UINT_PTR id, const wchar_t *text,
                         bool enabled = true) {
  UINT flags = MF_OWNERDRAW | (enabled ? MF_ENABLED : MF_GRAYED);
  AppendMenuW(menu, flags, id,
              reinterpret_cast<LPCWSTR>(addMenuDrawText(text, false)));
}
...
HMENU setupMenus(HWND hwnd) {
  ...
  // Keep the main menu order stable, including empty categories reserved for
  // future modules. Modules can opt into a category through menuParent.
  constexpr const wchar_t *kTopLevelMenuNames[] = {
      L"检验管理", L"工具", L"统计分析管理", L"定时任务", L"系统",
  };
  for (const wchar_t *name : kTopLevelMenuNames) {
    subMenus[subCount] = CreatePopupMenu();
    subNames[subCount] = name;
    AppendMenuW(bar, MF_POPUP | MF_OWNERDRAW, (UINT_PTR)subMenus[subCount],
                reinterpret_cast<LPCWSTR>(addMenuDrawText(name, true)));
    ++subCount;
  }
  ...
  for (int i = 0; i < g_moduleCount; i++) {
    ...
    for (int j = 0; j < subCount; j++) {
      if (wcscmp(subNames[j], m.menuParent) == 0) {
        idx = j;
        break;
      }
    }
    if (idx < 0) {
      ...
      AppendMenuW(
          bar, MF_POPUP | MF_OWNERDRAW, (UINT_PTR)subMenus[subCount],
          reinterpret_cast<LPCWSTR>(addMenuDrawText(m.menuParent, true)));
      idx = subCount++;
    }
    appendOwnerDrawItem(subMenus[idx], static_cast<UINT_PTR>(m.menuId),
                        m.menuLabel);
  }

  // Avoid presenting a blank popup until the first scheduled-task module is
  // registered. The placeholder is disabled and carries no command.
  for (int j = 0; j < subCount; j++) {
    if (wcscmp(subNames[j], L"定时任务") == 0 &&
        GetMenuItemCount(subMenus[j]) == 0) {
      appendOwnerDrawItem(subMenus[j], 0, L"功能预留", false);
      break;
    }
  }
...
void updateTimePane(HWND hwnd) {
  ...
  GetLocalTime(&st);
  wchar_t buf[64];
  swprintf(buf, 64, L"当前时间：%d年%d月%d日 %d:%02d:%02d", st.wYear, st.wMonth,
...
std::wstring getLocalIp() {
  ...
  if (GetAdaptersInfo(p, &bufLen) != ERROR_SUCCESS)
    return L"0.0.0.0";
  while (p) {
...
void setupStatusBar(HWND hwnd) {
  HWND sb = CreateWindowExW(0, STATUSCLASSNAMEW, L"", WS_CHILD | WS_VISIBLE, 0,
                            0, 0, 0, hwnd, win32_control_id(ID_STATUS),
  ...
  SendMessageW(sb, SB_SETTEXT, MAKEWPARAM(2, SBT_NOBORDERS),
               (LPARAM)ip.c_str());
}

bool measureMenuItem(LPMEASUREITEMSTRUCT mi) {
  if (!mi || mi->CtlType != ODT_MENU || !mi->itemData)
    return false;
  auto *data = reinterpret_cast<MenuDrawText *>(mi->itemData);
  HDC dc = GetDC(g_ctx.mainWindow);
  HFONT old = g_ctx.menuFont
                  ? static_cast<HFONT>(SelectObject(dc, g_ctx.menuFont))
                  : nullptr;
  RECT textRc{0, 0, 0, 0};
  ...
  mi->itemWidth = (std::max)(textRc.right - textRc.left + horizontalPad,
                             tm.tmAveCharWidth * 4);
...
bool drawMenuItem(LPDRAWITEMSTRUCT di) {
  if (!di || di->CtlType != ODT_MENU || !di->itemData)
    return false;
  ...
  HFONT old = g_ctx.menuFont
                  ? static_cast<HFONT>(SelectObject(di->hDC, g_ctx.menuFont))
                  : nullptr;
  SetBkMode(di->hDC, TRANSPARENT);
  const bool disabled = (di->itemState & (ODS_DISABLED | ODS_GRAYED)) != 0;
  SetTextColor(di->hDC, GetSysColor(selected ? COLOR_HIGHLIGHTTEXT
  ...
  DrawTextW(di->hDC, data->text.c_str(), -1, &textRc,
            DT_SINGLELINE | DT_VCENTER |
                (data->topLevel ? DT_CENTER : DT_LEFT));
  if (old)
...
LRESULT CALLBACK wndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
  ...
  case WM_CREATE: {
    ...
    g_ctx.mdiClient = CreateWindowExW(
        WS_EX_CLIENTEDGE, L"MDICLIENT", L"",
        WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_VSCROLL | WS_HSCROLL, 0, 0,
        0, 0, hwnd, nullptr, g_ctx.instance, &ccs);
    ...
    start_scheduled_result_check_monitor(hwnd, makeCtx());
    SetTimer(hwnd, ID_SCHEDULED_CHECK_TIMER, 60000, nullptr);
  ...
  case WM_SIZE: {
    ...
    if (tb)
      MoveWindow(tb, 0, 0, LOWORD(lp), tbH, TRUE);

    RECT sbRc{};
    HWND sb = GetDlgItem(hwnd, ID_STATUS);
    if (sb)
      GetWindowRect(sb, &sbRc);
    int sbH = sb ? (sbRc.bottom - sbRc.top) : 24;
    if (g_ctx.mdiClient)
      MoveWindow(g_ctx.mdiClient, 0, tbH, LOWORD(lp), HIWORD(lp) - tbH - sbH,
                 TRUE);

    if (sb)
      updateStatusBarParts(sb, LOWORD(lp));
    return 0;
  }
  case WM_MEASUREITEM:
    if (measureMenuItem(reinterpret_cast<LPMEASUREITEMSTRUCT>(lp)))
      return TRUE;
    break;
  case WM_DRAWITEM:
    if (drawMenuItem(reinterpret_cast<LPDRAWITEMSTRUCT>(lp)))
      return TRUE;
    break;
  case WM_COMMAND: {
    int id = LOWORD(wp);
    ...
    if (dispatchToolbarOnlyModule(id))
      return 0;

    // Fixed items
    switch (id) {
    case IDM_ABOUT: {
      const std::wstring aboutText = L"LIS 工作台\n版本 " +
                                     search::utf8_to_wide(search::kVersion) +
                                     L"\n\n作者：Zhao Wang";
      MessageBoxW(hwnd, aboutText.c_str(), L"关于", MB_ICONINFORMATION);
      return 0;
    }
    case ID_BTNCLOSE:
      closeActiveMdiChild();
    ...
    case IDM_EXIT:
      ...
      return 0;
    case IDM_CASCADE:
      SendMessageW(g_ctx.mdiClient, WM_MDICASCADE, 0, 0);
      return 0;
    case IDM_TILE_H:
      SendMessageW(g_ctx.mdiClient, WM_MDITILE, MDITILE_HORIZONTAL, 0);
      return 0;
    case IDM_TILE_V:
      SendMessageW(g_ctx.mdiClient, WM_MDITILE, MDITILE_VERTICAL, 0);
      return 0;
    case IDM_ARRANGE:
      SendMessageW(g_ctx.mdiClient, WM_MDIICONARRANGE, 0, 0);
      return 0;
    case IDM_CLOSE_ACTIVE:
      closeActiveMdiChild();
      return 0;
    }
    break;
  ...
  case app::WM_APP_SETTINGS_CHANGED:
    ...
    return 0;
  case WM_SCHEDULED_RESULT_NOTIFICATION:
    handle_scheduled_result_check_notification(lp);
    return 0;
  case WM_CLOSE:
    DestroyWindow(hwnd);
    return 0;
  case WM_DESTROY:
    ...
    KillTimer(hwnd, ID_AUTO_UPDATE_TIMER);
    KillTimer(hwnd, ID_SCHEDULED_CHECK_TIMER);
    stop_scheduled_result_check_monitor();
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
  ...
  applog::init(
      ...
      static_cast<unsigned>((std::max)(
          1, search::load_module_int(L"Logging", L"RetentionDays", 14))),
      static_cast<unsigned long long>((std::max)(
          1, (std::min)(1024,
                        search::load_module_int(L"Logging", L"MaxMiB", 20)))) *
          1024 * 1024);
  ...
  HWND hwnd = CreateWindowExW(0, wc.lpszClassName, L"LIS 工作台",
  ...
  if (g_ctx.uiFont)
    DeleteObject(g_ctx.uiFont);
  if (g_ctx.menuFont)
    DeleteObject(g_ctx.menuFont);
  return 0;
}
