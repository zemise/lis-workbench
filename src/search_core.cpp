...
bool run_auto_delete_crp_cycle(const AutoDeleteCrpCycleQuery& query,
    ...
    (void)query;
    (void)log;
    error = "run_auto_delete_crp_cycle is only available on Windows";
    return false;
#else
    ...
    const auto numeric_list = [&](const std::vector<std::string>& values,
                                  size_t begin, size_t end,
                                  std::string& out) {
        out.clear();
        for (size_t i = begin; i < end; ++i) {
            const std::string value = trim(values[i]);
    ...
    const auto fetch_ok = [](SQLRETURN rc) {
        return rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO;
    };

    if (trim(query.day_start).empty() || trim(query.day_end).empty()) {
        error = "auto delete CRP date range is empty";
        return false;
    }
    if (!query.catch_up && !numeric(query.last_scanned_rep_no)) {
        error = "auto delete CRP watermark is not numeric";
        return false;
    }

    const int report_batch_size = (std::max)(1, (std::min)(1000, query.report_batch_size));
    const int sql_batch_size = (std::max)(1, (std::min)(500, query.sql_batch_size));
    const std::string start = sql_escape(trim(query.day_start));
    const std::string end = sql_escape(trim(query.day_end));

    DbContext db;
    if (!connect(query.connection_string, db, error, log)) return false;

    auto execute_report_query = [&](const std::string& sql,
                                    std::vector<AutoDeleteCrpReportState>& rows,
                                    std::vector<std::string>* group_codes) {
        SQLHSTMT stmt = SQL_NULL_HSTMT;
        if (!exec_query(db.dbc, sql, stmt, error)) return false;
        SQLRETURN rc = SQL_SUCCESS;
        while (fetch_ok(rc = SQLFetch(stmt))) {
            AutoDeleteCrpReportState row;
            row.rep_no = trim(fetch_column(stmt, 1));
            std::string group_code;
            if (group_codes) group_code = trim(fetch_column(stmt, 2));
            row.patient_info_complete = trim(fetch_column(stmt, group_codes ? 3 : 2)) == "1";
            if (!numeric(row.rep_no)) {
                SQLFreeHandle(SQL_HANDLE_STMT, stmt);
                error = "auto delete CRP query returned a non-numeric REP_NO";
                return false;
            }
            rows.push_back(std::move(row));
            if (group_codes) group_codes->push_back(std::move(group_code));
        }
        if (rc != SQL_NO_DATA) {
    ...
    std::string next_watermark = trim(query.last_scanned_rep_no);
    if (query.catch_up) {
        std::ostringstream max_sql;
        max_sql << "SELECT CAST(ISNULL(MAX(r.REP_NO),0) AS varchar(20))"
                << " FROM LS_AS_REPORT r"
                << " WHERE r.CHK_DATE >= '" << start << "'"
                << " AND r.CHK_DATE < '" << end << "'";
        SQLHSTMT stmt = SQL_NULL_HSTMT;
        if (!exec_query(db.dbc, max_sql.str(), stmt, error)) return false;
        const SQLRETURN fetch_rc = SQLFetch(stmt);
        if (!fetch_ok(fetch_rc)) {
            error = "auto delete CRP failed to read snapshot watermark";
            SQLFreeHandle(SQL_HANDLE_STMT, stmt);
            return false;
        }
        next_watermark = trim(fetch_column(stmt, 1));
        SQLFreeHandle(SQL_HANDLE_STMT, stmt);
        if (!numeric(next_watermark)) {
            error = "auto delete CRP snapshot watermark is not numeric";
            return false;
        }

        std::string cursor = "0";
        while (true) {
            std::vector<AutoDeleteCrpReportState> page;
            std::ostringstream sql;
            sql << "SELECT TOP (" << report_batch_size << ")"
                << " CAST(r.REP_NO AS varchar(20)),"
                << " CASE WHEN NULLIF(LTRIM(RTRIM(r.NAME)),'') IS NOT NULL"
                << " AND NULLIF(LTRIM(RTRIM(r.SEX)),'') IS NOT NULL"
                << " AND NULLIF(LTRIM(RTRIM(r.AGE)),'') IS NOT NULL"
                << " THEN 1 ELSE 0 END"
                << " FROM LS_AS_REPORT r"
                << " WHERE r.REP_NO > " << cursor
                << " AND r.REP_NO <= " << next_watermark
                << " AND r.CHK_DATE >= '" << start << "'"
                << " AND r.CHK_DATE < '" << end << "'"
                << " AND r.DELETE_BIT=0 AND r.GROUP_CODE=101101"
                << " ORDER BY r.REP_NO";
            if (!execute_report_query(sql.str(), page, nullptr)) return false;
            if (page.empty()) break;
            cursor = page.back().rep_no;
            result.discovered_reports.insert(result.discovered_reports.end(),
                                             page.begin(), page.end());
            if (static_cast<int>(page.size()) < report_batch_size) break;
        }
    } else {
        std::vector<AutoDeleteCrpReportState> page;
        std::vector<std::string> group_codes;
        std::ostringstream sql;
        sql << "SELECT TOP (" << report_batch_size << ")"
            << " CAST(r.REP_NO AS varchar(20)), CAST(r.GROUP_CODE AS varchar(20)),"
            << " CASE WHEN NULLIF(LTRIM(RTRIM(r.NAME)),'') IS NOT NULL"
            << " AND NULLIF(LTRIM(RTRIM(r.SEX)),'') IS NOT NULL"
            << " AND NULLIF(LTRIM(RTRIM(r.AGE)),'') IS NOT NULL"
            << " THEN 1 ELSE 0 END"
            << " FROM LS_AS_REPORT r"
            << " WHERE r.REP_NO > " << trim(query.last_scanned_rep_no)
            << " AND r.CHK_DATE >= '" << start << "'"
            << " AND r.CHK_DATE < '" << end << "'"
            << " AND r.DELETE_BIT=0 ORDER BY r.REP_NO";
        if (!execute_report_query(sql.str(), page, &group_codes)) return false;
        for (size_t i = 0; i < page.size(); ++i) {
            next_watermark = page[i].rep_no;
            if (group_codes[i] == "101101") result.discovered_reports.push_back(page[i]);
        }
    }

    std::vector<std::string> pending;
    std::set<std::string> seen_pending;
    for (const auto& value : query.pending_rep_nos) {
        const std::string rep_no = trim(value);
        if (!numeric(rep_no)) {
            error = "auto delete CRP pending REP_NO is not numeric";
            return false;
        }
        if (seen_pending.insert(rep_no).second) pending.push_back(rep_no);
    }

    for (size_t begin = 0; begin < pending.size(); begin += static_cast<size_t>(sql_batch_size)) {
        const size_t batch_end = (std::min)(pending.size(), begin + static_cast<size_t>(sql_batch_size));
        std::string list;
        if (!numeric_list(pending, begin, batch_end, list)) {
            error = "auto delete CRP failed to build pending REP_NO list";
            return false;
        }
        std::ostringstream sql;
        sql << "SELECT CAST(r.REP_NO AS varchar(20)),"
            << " CASE WHEN NULLIF(LTRIM(RTRIM(r.NAME)),'') IS NOT NULL"
            << " AND NULLIF(LTRIM(RTRIM(r.SEX)),'') IS NOT NULL"
            << " AND NULLIF(LTRIM(RTRIM(r.AGE)),'') IS NOT NULL"
            << " THEN 1 ELSE 0 END"
            << " FROM LS_AS_REPORT r WHERE r.REP_NO IN (" << list << ")"
            << " AND r.CHK_DATE >= '" << start << "'"
            << " AND r.CHK_DATE < '" << end << "'"
            << " AND r.DELETE_BIT=0 AND r.GROUP_CODE=101101";
        if (!execute_report_query(sql.str(), result.pending_report_states, nullptr)) return false;
    }

    std::map<std::string, bool> complete_by_report;
    for (const auto& row : result.discovered_reports) {
        complete_by_report[row.rep_no] = row.patient_info_complete;
    }
    for (const auto& row : result.pending_report_states) {
        complete_by_report[row.rep_no] = row.patient_info_complete;
    }
    std::vector<std::string> ready_reports;
    for (const auto& entry : complete_by_report) {
        if (entry.second) ready_reports.push_back(entry.first);
    }

    for (size_t begin = 0; begin < ready_reports.size(); begin += static_cast<size_t>(sql_batch_size)) {
        const size_t report_end = (std::min)(ready_reports.size(), begin + static_cast<size_t>(sql_batch_size));
        std::string report_list;
        if (!numeric_list(ready_reports, begin, report_end, report_list)) {
            error = "auto delete CRP failed to build ready REP_NO list";
            return false;
        }
        std::vector<std::string> entry_ids;
        std::ostringstream detail_sql;
        detail_sql << "SELECT CAST(e.ID AS varchar(30)) FROM LS_AS_REPENTRY e"
                   << " WHERE e.REP_NO IN (" << report_list << ")"
                   << " AND e.ITEM_CODE=91089 AND e.DELETE_BIT=0";
        SQLHSTMT detail_stmt = SQL_NULL_HSTMT;
        if (!exec_query(db.dbc, detail_sql.str(), detail_stmt, error)) return false;
        SQLRETURN detail_rc = SQL_SUCCESS;
        while (fetch_ok(detail_rc = SQLFetch(detail_stmt))) {
            const std::string id = trim(fetch_column(detail_stmt, 1));
            if (!numeric(id)) {
                SQLFreeHandle(SQL_HANDLE_STMT, detail_stmt);
                error = "auto delete CRP query returned a non-numeric entry ID";
                return false;
            }
            entry_ids.push_back(id);
        }
        if (detail_rc != SQL_NO_DATA) {
            error = "SQLFetch failed: " + collect_diag(SQL_HANDLE_STMT, detail_stmt);
            SQLFreeHandle(SQL_HANDLE_STMT, detail_stmt);
            return false;
        }
        SQLFreeHandle(SQL_HANDLE_STMT, detail_stmt);

        for (size_t id_begin = 0; id_begin < entry_ids.size();
             id_begin += static_cast<size_t>(sql_batch_size)) {
            const size_t id_end = (std::min)(entry_ids.size(),
                                              id_begin + static_cast<size_t>(sql_batch_size));
            std::string id_list;
            if (!numeric_list(entry_ids, id_begin, id_end, id_list)) {
                error = "auto delete CRP failed to build entry ID list";
                return false;
            }
            std::ostringstream update_sql;
            update_sql << "UPDATE e SET e.DELETE_BIT=1"
                       << " FROM LS_AS_REPENTRY e INNER JOIN LS_AS_REPORT r"
                       << " ON r.REP_NO=e.REP_NO"
                       << " WHERE e.ID IN (" << id_list << ")"
                       << " AND e.ITEM_CODE=91089 AND e.DELETE_BIT=0"
                       << " AND r.GROUP_CODE=101101"
                       << " AND r.CHK_DATE >= '" << start << "'"
                       << " AND r.CHK_DATE < '" << end << "'"
                       << " AND r.DELETE_BIT=0"
                       << " AND NULLIF(LTRIM(RTRIM(r.NAME)),'') IS NOT NULL"
                       << " AND NULLIF(LTRIM(RTRIM(r.SEX)),'') IS NOT NULL"
                       << " AND NULLIF(LTRIM(RTRIM(r.AGE)),'') IS NOT NULL";
            SQLHSTMT update_stmt = SQL_NULL_HSTMT;
            if (!exec_query(db.dbc, update_sql.str(), update_stmt, error)) return false;
            SQLLEN affected = 0;
            const SQLRETURN row_count_rc = SQLRowCount(update_stmt, &affected);
            if ((row_count_rc == SQL_SUCCESS || row_count_rc == SQL_SUCCESS_WITH_INFO) &&
                affected > 0) {
                result.updated_entry_count += static_cast<int>(affected);
            }
            SQLFreeHandle(SQL_HANDLE_STMT, update_stmt);
        }
    }

    result.last_scanned_rep_no = next_watermark.empty() ? "0" : next_watermark;
    error.clear();
    return true;
#endif
}
