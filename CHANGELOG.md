...
- **质控分析模块**：新增 `统计分析管理 -> 质控分析` 页面，基于系统设置中的 `仪器 + 固定样本号` 配置从 LIS 直查质控结果，并在内存中完成分组、统计、规则判断、图表和导出；本地 SQLite 仅用于保存质控配置，不作为质控结果主数据源。
- **质控品设置**：系统设置页新增质控品设置区域，支持维护仪器、固定样本号、质控名称、水平、项目代码、靶值、SD 和启用状态；仪器选择复用常规报告仪器 picker，避免质控设置和常规报告使用不同仪器来源。
- **LIS 质控查询**：按 `LS_AS_REPORT.MACH_CODE + OPER_NO` 精确匹配质控品配置，从 `LS_AS_REPORT + LS_AS_REPENTRY` 查询质控点，并通过 `LS_AS_REPORT.OPER_CODE -> JC_EMPLOYEE_PROPERTY.NAME` 显示检验者；质控分析页默认不自动查询，用户选择仪器并点击 `查询/刷新 LIS` 后访问 LIS。
- **Levey-Jennings 图**：新增原生 Win32 L-J 图窗口，支持均值线、`±1SD / ±2SD / ±3SD` 参考线、多水平纵向堆叠、滚动查看、点位 tooltip、点击图点定位下方明细、选择明细高亮图点；图表窗口按当前显示器工作区居中打开并随窗口尺寸重绘。
- **Westgard 规则判断**：质控点支持 Z 值、状态和命中规则，已覆盖 `1-2s / 1-3s / 2-2s / R-4s / 4-1s / 10x`；规则计算优先使用配置靶值和 SD，未配置时使用当前查询周期内同分组统计值；`4-1s` 和 `10x` 首版按警告处理，已有失控状态不降级。
- **质控概览和筛选**：右侧今日质控概览新增摘要条和卡片化信息列表，显示分组、失控、警告、在控、未判定、明细和点数摘要；概览行支持单击同步左侧分组和下方明细，双击打开对应 L-J 图；状态筛选支持 `全部 / 警告+失控 / 仅失控 / 仅警告`，筛选基于当前内存结果完成，不重新查询 LIS。
