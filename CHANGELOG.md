...
- **常规报告自动删除 CRP**：仪器代码为 `1011` 或其主项目代码为 `101101` 时，右侧列表右键菜单新增全局 `自动删除CRP` 开关；开启状态使用原生勾选并保存到本机配置，LIS 工作台重启后自动恢复，关闭常规报告子窗口不影响后台任务继续运行。
- **增量扫描与延迟数据保护**：主窗口级服务每 60 秒按 `LS_AS_REPORT.CHK_DATE` 扫描当天 `GROUP_CODE=101101` 的报告，使用单调递增 `REP_NO` 水位减少重复读取；新报告进入短期复查队列，人口学信息首次不完整时仅保留 30 分钟，首次完整时保留 60 分钟，兼顾延迟写入和数据库负载。
- **严格更新边界**：仅处理姓名、性别、年龄均非空的报告，并把对应 `LS_AS_REPENTRY.ITEM_CODE=91089 AND DELETE_BIT=0` 的明细更新为 `DELETE_BIT=1`；其他项目明细和 `LS_AS_REPORT` 主记录保持不变。数据库失败只写程序日志，后续周期继续重试，任务不会并发重叠。
- **仪器项目解析收敛**：快捷仪器不新增主项目代码配置项；常规报告页面只保留运行期 `selectedMachineGroupCode`，应用快捷仪器时复用现有仪器查询结果，按科室代码与仪器代码解析最新 `GROUP_CODE`。
- Windows MinGW 主程序构建通过；实际 LIS 数据库运行效果留待现场运行继续验证。

## v2026.09.14

- **免疫重复项目默认时间范围**：签收时间默认改为含当天在内最近 7 个自然日，即 6 天前 `00:00` 至当天 `23:59`，仍支持用户手动调整。
- **免疫重复项目统计患者工作台**：主操作统一命名为“查询重复医嘱”，顶部增加“选择时间 → 查询 → 选择患者核对”的三步提示；主列表按“一位涉及患者一行”呈现，选中患者后在下方直接对照已有基准医嘱与重复医嘱。绿色标识基准、蓝色标识同条码重复、橙色标识跨条码重复，汇总指标使用同系浅色；支持关系筛选和患者搜索。YZXMID 技术配置和当前时间范围的项目记录统一移入“规则设置”。
- **免疫重复项目统计代码化配置**：基准和重复项目改为只按用户配置的 `YJ_ZYSQ.YZXMID` 集合匹配，不保留 `SQNR` 文字兜底；默认基准 `12731`、默认重复 `8714;8724;7345`，支持本机持久化。新增当前时间段项目记录窗口，展示 `YZXMID / SQNR / 记录数 / 最近签收时间` 并可加入两组配置；重复查询移除相关 `EXISTS`，两批代码窄结果在客户端按 `INPATIENT_ID` 关联。
