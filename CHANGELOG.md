...
- **输血 LIS 查询性能优化**：报告列表新增 `科室代码`、`仪器代码`，并改用专用轻量查询 `query_blood_lis_reports()`；按病人号查询时直接过滤 `LS_AS_REPORT.REG_NO`，避开通用报告查询中的医嘱聚合、无关字典联查和条码表子查询，查询完成状态栏显示命中血型/血常规仪器配置的报告数量。
