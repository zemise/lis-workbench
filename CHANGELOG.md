...
- **常规报告条码打印入口**：底部“打印（F4）”改为“打印条码”，直接打印当前报告对应的样本条码，并复用右侧报告列表的既有打印链路。
- **常规报告批量条码打印**：底部“批打印”改为“批打印条码”，按当前检验日期和仪器的已加载报告手动输入起止样本号；输入过程中即时刷新预览，预览页支持逐条取消，空条码不可选，完全相同的打印载荷标记疑似重复并默认不选，超过 50 张时二次确认。样本号范围和列表默认排序统一使用支持前导零、超长数字和字母数字混合的自然顺序。勾选批打和范围批打共用后台顺序提交任务，界面显示进度；遇到首个失败即停止，明确报告已入队、失败样本号和未尝试数量，不自动重试。
- **范围预览布局优化**：预览弹窗改为按客户区与 DPI 动态排列输入框、列表、汇总信息和底部按钮，避免元素重叠；移除独立“预览”按钮，键入、粘贴或删除起止样本号时自动刷新结果。
