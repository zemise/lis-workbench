...
- 主程序 `输血结果查询` 模块：
  ...
  - `查询检验结果` 窗口右侧报告列表的 `样本号` 与检验结果查询页一致，来自 `LS_AS_REPORT.OPER_NO`，位于 `检验时间` 前。
...
9. 可双击第一张报告列表中的报告行跳转到 `常规报告` 页面，并自动定位同一条报告。
10. 如果本次查询使用了病人姓名或病人号，可点击 `趋势图` 查看该查询条件下的项目趋势。
11. 在趋势图窗口右侧点击项目行切换图表，勾选项目后可导出对应趋势明细 CSV。
