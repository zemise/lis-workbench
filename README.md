...
- 统计分析管理菜单新增 `质控分析` 页面：用户先在系统设置中维护某台检验仪器的固定质控样本号、质控名称和水平；质控品设置区采用左侧配置列表、右侧双列表单，读取日期、批号开始日期和结束日期均使用日历选择器，其中结束日期允许为空；可按 `仪器 + 样本号 + 指定日期` 只读读取 LIS 项目清单并保存为本机质控项目，重复读取会复用已有样本配置并更新本机项目清单。质控批号按样本号维护开始日期、可空结束日期，以及该批号下各项目独立靶值和 SD；质控分析页从右侧操作侧栏选择日期、仪器、水平和状态并点击 `查询/刷新 LIS` 后，按 `LS_AS_REPORT.MACH_CODE + OPER_NO` 精确从 LIS 只读查询质控结果。页面在内存中按结果日期匹配当时生效的本机批号和项目靶值，完成分组、均值/SD/CV%、Z 值和 Westgard 规则判断，已覆盖 `1-2s / 1-3s / 2-2s / R-4s / 4-1s / 10x`；主区域采用卡片优先看板，摘要条按项目统计失控、警告、在控、无数据和未判定数量，卡片标题按 `（项目英文名）项目名` 显示并用当前质控日的结果、操作人和水平圆点呈现状态；卡片标题栏单击打开 L-J 图，正文单击只选择卡片；右侧侧栏提供 `L-J图 / 明细 / 导出` 和项目列表，底部明细默认收起并可按需展开；L-J 图支持多水平纵向堆叠、滚动、点位 tooltip、点击图点定位明细和明细高亮图点。质控样本、项目、批号和项目靶值保存在本机 SQLite，质控结果事实来源以 LIS 为准且不写 LIS。
