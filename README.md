...
当前源码版本：`v2026.09.24`（开发分支，尚未打 tag 发布）

2026-09-15 的阶段性交接记录见 [持续优化路线图](PROJECT_OPTIMIZATION_ROADMAP.md#2026-09-15-暂停与恢复交接)；其中的版本与发布状态属于当时的历史记录。本分支继续开发定时核查功能，正式 tag / Release 另行执行。
...
- 菜单栏新增 `定时任务 -> 定时核查结果`：用户从 `LS_AS_ITEM` 项目字典选择项目后，可配置双项目数值比较或项目与固定值（如 `> 90.0`）的单项目阈值监测，直接输入完整 `ITEM_CODE` 时会按字典精确匹配并回填项目名称、英文名和单位；主程序运行期间每分钟只读核查当日 `LS_AS_REPORT + LS_AS_REPENTRY` 中规则涉及的项目，按同一 `REP_NO` 配对并严格解析纯数值结果。启动、规则变更、跨日及手动扫描覆盖当天全量；常规轮次使用 `REP_NO` 增量、未完成报告复查和分段轮巡，以减少重复读取，同时保留旧报告结果更正的兜底检查。首次扫描不按审核或发送状态过滤，并直接提醒已有新命中；命中记录、观察指纹、规则和扫描进度保存在独立 SQLite，相同结果指纹不重复提醒。新命中通过 Windows 通知区域合并提醒，同时播放一次提示音并闪烁任务栏；全部未处理记录跨日期累计在桌面右下角非抢焦点的置顶悬浮窗中，可展开查看最近记录，拖动标题区域可调整并持久化摆放位置且限制在当前显示器工作区内，展开/收起保持底边固定；点击记录会直接跳转对应常规报告并以橙色背景标记异常项目行，点击页脚仍打开待处理列表，全部处理后自动隐藏。页面顶部以“待处理 / 规则设置”页签组织内容，待处理页支持按样本号或报告号搜索、点击表头排序和“全部标记已处理”，规则列表可直接勾选启用/停用；未处理行使用醒目底色，规则修改或删除时同步清除旧命中和观察指纹。页面支持规则增删改、立即扫描、命中处理及双击跳转常规报告，扫描期间显示进度提示。空值及 `阴性 / 1+ / <10` 等非纯数值首版跳过，不写 LIS。
- 定时核查悬浮窗现以较窄的 390 逻辑像素布局显示样本号、核查关系和逐条“标记已处理”按钮；关系中的项目优先显示英文名，缺失时依次回退中文名、项目代码。项目字典按数据库连接缓存 30 分钟，扫描结果不再逐次关联字典表。停用规则会清除其旧提醒；升级前已存在的提醒不因重复扫描重建，更新规则后产生的新提醒才会保存英文名。悬浮窗跳转会最大化目标页面，异常项目高亮仅在对应报告内有效。悬浮窗同时适配 Win7–Win11 系统外观：`CS_DROPSHADOW` 系统阴影、Win11 系统圆角、系统 UI 字体和系统强调色，不支持的旧系统回退原橙红配色。
- 定时核查新规则须选择检验科室与仪器；同仪器规则合并为一次当日目标项目查询，每分钟完整回看该仪器，避免最近 100 个 `REP_NO` 漏检。旧规则未限定仪器时继续沿用上条所述的全局增量、待复查和轮巡路径，修改保存前需主动绑定仪器。规则项目仅可从该仪器在 `LS_AS_GROUP_ITEM` 配置且存在于 `LS_AS_ITEM` 字典的项目中选择，输入项目代码也受同一范围约束；映射加载失败或无可选项目时禁止保存，不查询 `LS_AS_ADDITEM`。规则表单在宽窗口采用双列、窄窗口逐行排布，按钮可换行。LIS 只读，规则仍是各工作站本地保存、不跨机共享。
- 自定义工具栏中的 `常规报告` 已替换原 `工具2` 占位页，按 `temp/模版2.png` 基本完成三栏报告工作台界面：
  ...
  - 右侧信息列表第一列支持勾选；行右键菜单提供 `打印条码`、`打印勾选条码` 和 `趋势图`，底部 `批打印条码` 可在当前检验日期和仪器的已加载报告内指定起止样本号，输入过程中即时预览实际命中项并取消个别记录；空条码不可选，疑似重复默认不选，超过 50 张时二次确认。条码打印会把对应行样本号、组合项目、条码号、姓名、标本、开单日期、科室代码、病人号填入 LabelPrint 的 `MedicalLabelData`，再交给共享条码打印 helper 发送到条码打印机；其中条码上的组合项目取自该右侧报告行的 `检验仪器` 列内容，开单日期按 `2026/5/21` 这种仅日期格式打印。批量任务在后台顺序提交，遇到首个失败即停止且不自动重试。右键 `趋势图` 和底部 `趋势图` 按钮复用检验结果查询页趋势图窗口，按当前报告的病人号/姓名、当前检验仪器和科室生成查询条件，窗口顶部提供开始/结束日期选择器和刷新按钮，默认结束日期为当前报告检验日期，开始日期为前 14 天；窗口按父窗口所在显示器工作区居中打开。
