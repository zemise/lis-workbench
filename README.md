...
- 统计分析管理菜单提供 `免疫重复项目统计(&3)`：主操作准确命名为“查询重复医嘱”，签收时间默认含当天在内的最近 7 个自然日；顶部给出三步使用提示，并保留规则设置、同/跨条码筛选和患者搜索。结果按 `INPATIENT_ID` 聚合为“一位涉及患者一行”，选中患者后在下方按“已有基准医嘱 → 重复医嘱”对照展示；绿色表示基准，蓝色表示同条码重复，橙色表示跨条码重复。基准和重复项目只按用户配置的 `YZXMID` 集合匹配，不使用 `SQNR` 文字兜底；技术代码与当前时间范围的项目记录统一收进规则设置窗口。
...
- 自定义工具栏中的 `常规报告` 已替换原 `工具2` 占位页，按 `temp/模版2.png` 基本完成三栏报告工作台界面：
  ...
  - 当前仪器代码为 `1011`，或其通过现有仪器查询结果解析出的主项目代码为 `101101` 时，右侧列表右键菜单显示全局开关 `自动删除CRP`。开启状态使用原生勾选并保存到本机配置，重启工作台后自动恢复；任务由主窗口持有，关闭常规报告页面后仍每 60 秒检查当天 `CHK_DATE` 范围内 `GROUP_CODE=101101`、姓名/性别/年龄完整的报告，仅将其 `ITEM_CODE=91089` 且尚未删除的明细 `DELETE_BIT` 更新为 `1`，不修改其他明细或报告主记录。扫描使用 `REP_NO` 水位减少重复读取，并将首次发现时人口学不完整/完整的报告分别放入固定 30/60 分钟的短期复查队列；数据库失败只记录日志并等待后续周期重试。
