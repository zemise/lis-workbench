...
- 工具菜单中的 `常规报告` 已替换原 `工具2` 占位页，按 `temp/模版2.png` 基本完成三栏报告工作台界面：
  ...
  - 右侧信息列表按验单号检查是否存在 `LS_AS_REPENTRY.NORMAL_WJ=9`，因此列表刷新后无需点选该行也能提示危急值；未审核且未发送显示 `#FAC0CB`，已审核且已发送显示 `#FFFF39`。已发送报告仍兼容 `LS_AS_WjzMessage` 未删除记录的黄色提示。
  - 右侧顶部第二行统计当前列表中的 `危急报告数` 和 `危急报告已审`；前者统计存在 `NORMAL_WJ=9` 的报告行，后者统计其中已审核且已发送的报告行。
