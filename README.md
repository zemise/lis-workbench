...
- 菜单栏新增 `定时任务 -> 定时核查结果`：用户从 `LS_AS_ITEM` 项目字典选择项目后，可配置双项目数值比较或项目与固定值（如 `> 90.0`）的单项目阈值监测，直接输入完整 `ITEM_CODE` 时会按字典精确匹配并回填项目名称、英文名和单位；主程序运行期间每分钟只读扫描当日 `LS_AS_REPORT + LS_AS_REPENTRY` 中规则涉及的项目，按同一 `REP_NO` 配对并严格解析纯数值结果。首次扫描包含当天全部报告，不按审核或发送状态过滤，并直接提醒已有新命中；命中记录、观察指纹和规则保存在独立 SQLite，相同结果指纹不重复提醒。新命中通过 Windows 通知区域合并提醒，同时播放一次提示音并闪烁任务栏；全部未处理记录跨日期累计在桌面右下角非抢焦点的置顶悬浮窗中，可展开查看最近记录，拖动标题区域可调整并持久化摆放位置且限制在当前显示器工作区内，展开/收起保持底边固定；点击记录会直接跳转对应常规报告并以橙色背景标记异常项目行，点击页脚仍打开待处理列表，全部处理后自动隐藏。页面默认收起规则设置并优先展示未处理记录，未处理行使用醒目底色；规则修改或删除时同步清除旧命中和观察指纹。页面支持规则增删改、启停、立即扫描、命中处理及双击跳转常规报告。空值及 `阴性 / 1+ / <10` 等非纯数值首版跳过，不写 LIS。
