...
| `scheduled_result_check_core.* / scheduled_result_check_store.* / scheduled_result_check_module.*` | `定时任务 -> 定时核查结果`；核心层严格解析纯数值并按同一 `REP_NO` 执行双项目比较或项目与固定值的阈值监测，存储层用独立 SQLite 保存规则、已观察指纹和提醒历史，模块层提供待处理优先页面、可折叠规则设置、项目字典选择、命中处理与常规报告跳转；主程序级调度器固定每分钟只读扫描当日目标项目，页面关闭后仍继续运行。新命中使用 Windows 通知区域、提示音和任务栏闪烁做一次性提醒，未处理记录跨日期累计在非激活置顶悬浮窗中；悬浮窗支持拖动及位置持久化，点击记录会跳转常规报告并高亮异常项目行，点击内容区其他位置仍打开待处理页面。规则修改或删除时，存储层在事务中清除该规则原有的命中和观察记录；单纯启停保留历史。设计与当前实现口径见 `SCHEDULED_RESULT_CHECK_DESIGN.md`。 |
