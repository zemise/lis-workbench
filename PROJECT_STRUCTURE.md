...
- `IMMUNE_DUPLICATE_ITEM_STATISTICS_DESIGN.md` — `统计分析管理 -> 免疫重复项目统计` 当前实现说明，记录患者核查界面、`YZXMID` 项目规则、`YJ_ZYSQ` 查询链路、同一 `INPATIENT_ID` 重复医嘱匹配和统计口径
...
| `immune_duplicate_statistics_module.cpp/h` | 免疫重复项目统计患者核查工作台；顶部为时间、查询、规则设置、关系筛选和患者搜索，主列表按 `INPATIENT_ID` 一位患者一行，下方对照已有基准医嘱与重复医嘱；技术代码和当前时间段项目记录收进规则设置窗口。查询只按 `YZXMID` 集合及 `JSSJ / ZXKS` 读取并按 `YJSQID` 去重，不使用 SQNR 文字兜底、不写业务表 |
