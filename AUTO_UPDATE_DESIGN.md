...
- 新增 `update_core` 静态库，包含 manifest 解析、版本比较、文件 SHA-256 校验和更新源抽象。
...
- 主程序系统设置页已新增更新源配置；菜单栏 `系统 -> 检查更新` 可按 `[Update]` 配置创建文件夹或 HTTP 更新源，在后台线程调用 `check_and_fetch_update`，把更新包缓存到 `ProgramData\LISWorkbench\UpdateCache`，并在用户确认后启动 `Updater.exe`、退出主程序。
- `Updater.exe` 每次运行都会自动写入安装目录 `log\updater.log`，用于现场更新失败后的问题分析。
...
- `manifestUrl`：例如 `https://github.com/zemise/lis-workbench/releases/latest/download/manifest.json`
- `baseUrl`：可选，用于解析相对包路径。
- GitHub Release 使用 `releases/latest/download/manifest.json` 固定 URL，客户端不需要随版本号变更配置。
...
- 启动后延迟检查，避免阻塞主界面创建。
- 每天最多检查一次，检查日期写入 `[Update] LastCheckTime`。
- 自动检查只读取 manifest 并提示新版本，不下载更新包，不自动安装。
- 用户确认后进入手动下载、校验和安装流程。
...
```text
C:\Program Files\LISWorkbench\log
  updater.log
```

说明：

- 下载缓存仍放在 `ProgramData`，避免安装目录缓存膨胀；更新日志固定写入安装目录下的 `log` 文件夹，便于现场按程序路径直接定位。
...
[Update]
SourceType=Folder
ManifestUrl=https://github.com/zemise/lis-workbench/releases/latest/download/manifest.json
...
- `SourceType`：`Http` 或 `Folder`；设置页默认选择 `Folder`。
- `ManifestUrl`：HTTP 更新源入口；设置页默认使用 GitHub latest manifest 地址，且仅在更新源为 `Http` 时显示。
- `FolderPath`：共享文件夹更新源入口，仅在更新源为 `Folder` 时显示。
- `AutoCheck`：`1` 时主程序启动后延迟自动检查更新；`0` 时不检查。
- `LastCheckTime`：自动检查日期，格式 `yyyyMMdd`，用于限制每天最多检查一次。
- `Channel`：首阶段固定 `stable`。
