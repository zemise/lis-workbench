...
当前实现采用原生 Win32 `ListView` 做“卡片化信息列表”，先避免复杂自绘卡片带来的 DPI、滚动和重绘成本：

- 右侧概览区顶部增加摘要条，汇总当前筛选范围内的分组数、失控/警告/在控/未判定分组数，以及明细、失控点、警告点数量。
- 右侧概览列表按 `质控项目 / 状态 / 最近结果 / 最近时间 / 点数 / 失控警告 / 仪器 / L-J图` 组织，优先突出状态和最近质控结果。
- 单击概览行时同步选择左侧分组并刷新下方明细。
- 双击概览行时打开对应分组的 L-J 图。
- 后续如原生列表视觉仍不足，再考虑 owner-draw 卡片网格；当前先保持原生控件稳定性和可维护性。
