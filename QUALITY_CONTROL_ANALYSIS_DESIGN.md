...
- 质控分析页打开后不自动查询；用户选择仪器后点击“查询”可优先使用有效 SQLite 缓存，点击“刷新 LIS”则强制按配置和日期范围查询 LIS。
- 查询结果在内存中聚合、排序、统计并展示。
- 展示质控明细表。
- 预留 Levey-Jennings 质控图入口，完整绘图放到阶段 2。
- 预留 Westgard 规则状态字段，完整规则判断放到阶段 3。
- 支持从明细双击回跳常规报告并定位原始报告。
- 在本机 SQLite 中维护质控批号、开始日期、可空结束日期、靶值和 SD；LIS 仍只作为质控结果事实来源。
...
│ L-J图 | 数据列表 | 规则判定 | 查询记录                     │
├────────────────────────────────────────────────────────────┤
│ 开始日期 结束日期  水平 L1 L2 L3  查询  刷新LIS  选项       │
├────────────────────────────────────────────────────────────┤
...
| 查询 | 按当前配置和日期范围优先读取有效 SQLite 缓存；缓存无效时后台查询 LIS |
| 刷新 LIS | 强制按当前配置和日期范围直接查询 LIS，完成后更新 SQLite 缓存 |
...
- 页面右侧参考 `参考1.png` 增加操作侧栏：上方放置日期、仪器、水平、状态和查询按钮；中部提供 `L-J图 / 明细 / 导出` 操作；下方项目列表可反向选中卡片。不再在侧栏顶部重复显示当前卡片的项目、状态、今日结果、操作人和水平状态。
...
- 卡片内展示项目、当前质控日的今日结果、操作人和状态，不再显示最近时间和点数。
...
| 层级 | 说明 |
| --- | --- |
| 质控样本配置 | `仪器 + 样本号 + 水平/质控名称`，表示这个固定样本号是某台仪器上的一个质控品 |
| 质控项目清单 | 该样本号下哪些 `LS_AS_REPENTRY.ITEM_CODE` 纳入质控分析，项目可从 LIS 指定日期结果读取后勾选保存 |
...
如果同一台仪器的同一个固定样本号需要区分多个质控水平，应优先要求用户配置不同样本号；如果现场确实存在同一样本号跨水平复用，需要在质控样本配置层用 `level` 或 `qc_name` 作为区分条件。
...
- Tab 左侧维护本机质控样本配置列表：仪器、样本号、质控名称、水平、当前批号。
...
- 项目清单通过 `读取项目` 按 `仪器 + 样本号 + 指定日期` 从 LIS 读取，并允许用户勾选要纳入质控的项目。
...
保存系统设置页维护的质控样本识别规则：某台仪器的某个固定样本号是一个质控品。这一层不保存项目靶值和 SD，也不随批号更换而重复创建。
...
```text
旧 qc_config 的 mach_code/sample_no/qc_name/level -> qc_sample_config
旧 qc_config 的 item_code/item_name/item_eng/unit -> qc_sample_item
...
保存从 LIS 查询到的结果明细缓存。LIS 仍是事实来源，SQLite 只作为单机加速层；命中相同查询条件且缓存仍有效时，可直接从 SQLite 读取并在界面上标识“本机缓存”。
...
质控分析页默认不自动访问 LIS。用户先选择仪器，再点击“查询”或“刷新 LIS”；查询应后台执行，完成后把结果放入内存模型，页面筛选、排序、绘图和导出优先基于当前内存结果完成。

本节所有 LIS SQL 都必须按只读查询实现。示例 SQL 只允许使用 `SELECT` 或只读 CTE；缓存、配置、批号、处理记录等任何写入 SQL 都必须走本机 SQLite 连接，不能走 LIS 连接。

推荐查询入口：

- 质控分析页增加“查询”和“刷新 LIS”两个动作；“查询”允许命中 SQLite 缓存，“刷新 LIS”强制访问 LIS。
- 页面打开后只初始化日期范围和列表，不自动查询 LIS。
- 未选择仪器时点击查询，应提示先选择检验仪器。
- 系统设置的“质控品设置”Tab 中可增加“测试查询最近 30 天”按钮，用于验证配置是否能命中 LIS 结果。
- 如果启用本机缓存，应在状态栏显示缓存命中、缓存过期、刷新 LIS 等状态，避免用户误以为缓存数据就是实时 LIS。

查询流程：

1. 读取启用的质控样本配置和项目清单。
2. 根据仪器、日期范围、水平、启用样本配置版本和项目清单版本生成 `cache_key`。
3. 如果用户点击“查询”，先判断 `qc_query_cache_meta` 中是否存在有效缓存。
4. 缓存有效时，从 `qc_result_cache` 读取明细；缓存无效、过期或用户点击“刷新 LIS”时，从 LIS 查询匹配结果。
5. LIS 查询结果成功返回后，upsert 到本机 SQLite 的 `qc_result_cache`，并更新本机 SQLite 的 `qc_query_cache_meta`。
6. 将结果逐条解析为内存中的 `QualityControlPoint`。
...
设置页读取后，项目列表默认只展示本次指定日期实际检出的项目。当前实现会按 `mach_code + sample_no + level` 复用已有 `qc_sample_config`，并将读取到的项目写入或更新 `qc_sample_item`；后续换批号时不再强制重新读取项目，除非现场项目组成变化。
...
| 动作 | 行为 |
| --- | --- |
| 查询 | 优先读取有效 SQLite 缓存；缓存不存在、过期或不完整时再查询 LIS 并回写缓存 |
| 刷新 LIS | 强制绕过缓存，重新查询 LIS，完成后覆盖更新 SQLite 缓存 |

界面必须明确显示当前数据来源：

```text
来源：LIS 实时查询
来源：SQLite 缓存，缓存时间 2026-06-22 09:30:00
缓存已过期，已刷新 LIS
缓存命中，可手动刷新 LIS
```
...
| 查询日期 | 缓存策略 |
| --- | --- |
| 包含当前日 | 缓存 5-10 分钟，过期后自动刷新 LIS |
| 全部早于当前日 | 优先复用缓存，可长期有效；用户仍可手动刷新 LIS |
...
2. 同步更新 `cache_key / cached_at / updated_at / last_seen_at`。
3. 同一 `cache_key` 下本次 LIS 没返回、但 SQLite 里曾存在的记录，只在本机 SQLite 中标记 `deleted_in_lis=1` 或从该缓存范围移除，避免 LIS 修改或删除后仍显示旧数据。
4. 更新本机 SQLite 中 `qc_query_cache_meta` 的 `row_count / latest_effective_time / max_entry_id / refreshed_at`。

第二阶段可增加 LIS 轻量指纹校验。相同查询条件下，先查询 LIS 指纹：

```sql
SELECT
  COUNT(*) AS row_count,
  MAX(CONVERT(varchar(19), COALESCE(r.REP_TIME, r.CHK_DATE, r.REP_DATE), 120)) AS latest_effective_time,
  MAX(CAST(e.ID AS varchar(30))) AS max_entry_id
FROM LS_AS_REPORT r WITH (NOLOCK)
JOIN LS_AS_REPENTRY e WITH (NOLOCK)
  ON e.REP_NO = r.REP_NO
 AND ISNULL(e.DELETE_BIT, 0) = 0
...
如果 LIS 指纹与 `qc_query_cache_meta` 一致，则直接读取 SQLite 明细；如果不一致，再拉取 LIS 全量明细并更新缓存。这样仍会访问一次 LIS，但访问成本远低于每次拉全量结果。
...
- 增加“查询”和“刷新 LIS”动作；前者可复用有效 SQLite 缓存，后者强制从 LIS 查询质控结果。
...
- 内存分组统计中的均值、SD、CV% 和 L-J 图参考线使用同一套规则基准，基准来源为 `靶值` 或 `本期`。
...
- 新增本机查询缓存：`查询` 优先命中有效 SQLite 缓存，`刷新 LIS` 强制访问 LIS。
- 第一版使用 TTL 策略：今日数据短缓存，历史数据优先复用缓存。
- LIS 查询成功后按 `source_rep_no + source_entry_key` upsert 到本机 SQLite 的 `qc_result_cache`，并维护 `qc_query_cache_meta`。
- 后续增加 LIS 轻量指纹校验，使用 `row_count + latest_effective_time + max_entry_id` 判断缓存是否仍与 LIS 一致。
...
8. LIS 查询性能
   直接查 LIS 简化多电脑一致性，但大日期范围、多仪器、多项目查询可能较慢。阶段 1 应先限制默认日期范围并后台查询；后续缓存阶段再用 SQLite 缓存相同查询条件下的 LIS 结果，并提供手动刷新 LIS。
...
不建议第一版写 LIS 业务表、做跨电脑批号共享、自动审核控制，或把 SQLite 作为多电脑共享主库。先把“能配置、能查 LIS、能分析、能画图、能判定、能追溯”做稳定，再根据现场性能和多电脑一致性需求，决定是否增加本机缓存、集中配置和正式报表能力。
